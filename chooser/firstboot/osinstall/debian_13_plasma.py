"""Debian 13 KDE Plasma — FBL-native unpack.

Catalog ``install``: ``debian-13-plasma``.
Do not reuse this file for Debian GNOME, Xfce, or later Debian versions.
Live identity is ``user``; the customer form is the installed user.
Copy Debian's shim, not Canonical GRUB. Display manager is SDDM.
"""
...
class Debian13Plasma:
    id = ID
    aliases = ALIASES
    default_hostname = "debian"
    display_name = "Debian 13 Plasma"
