#!/usr/bin/env python3
"""Debian 13 Cinnamon native installer."""
...
from firstboot.osinstall.debian_13_cinnamon import DRIVER, ID  # noqa: E402
...
class Debian13CinnamonTests(unittest.TestCase):
    def test_driver_contract(self) -> None:
        self.assertEqual(ID, "debian-13-cinnamon")
    ...
    def test_live_squashfs_relpaths(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-iso-")
        try:
    ...
    def test_configure_writes_customer_not_live_user(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-root-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-efi-")
        try:
            fake_tree(root)
            ...
            self.assertIn("greeter-session=slick-greeter", debian_drop)
            self.assertFalse(
                os.path.isfile(
                    os.path.join(root, "etc", "xdg", "autostart", "calamares.desktop")
                )
    ...
    def test_health_check_rejects_missing_lightdm(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-dm-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-dm-efi-")
        try:
            fake_tree(root)
            os.unlink(
                os.path.join(root, "usr", "lib", "systemd", "system", "lightdm.service")
    ...
    def test_iso_extras_signed_debs_not_live_grub(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-cinnamon-pool-")
        try:
            pool = os.path.join(
