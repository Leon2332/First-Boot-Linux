#!/usr/bin/env python3
"""Debian 13 GNOME native installer."""
...
from firstboot.osinstall.debian_13_gnome import DRIVER, ID  # noqa: E402
...
def fake_tree(root: str) -> None:
    ...
    with open(os.path.join(root, "etc", "gdm3", "daemon.conf"), "w", encoding="utf-8") as fh:
        fh.write(
            "[daemon]\n"
    ...
    with open(
        os.path.join(root, "etc", "gdm3", "daemon.conf.d", "live.conf"),
        "w",
        encoding="utf-8",
    ) as fh:
        fh.write("[daemon]\nAutomaticLoginEnable=True\nAutomaticLogin=user\n")
...
class Debian13GnomeTests(unittest.TestCase):
    def test_driver_contract(self) -> None:
        self.assertEqual(ID, "debian-13-gnome")
    ...
    def test_live_squashfs_relpaths(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-gnome-iso-")
        try:
    ...
    def test_configure_writes_customer_not_live_user(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-gnome-root-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-gnome-efi-")
        try:
            fake_tree(root)
            ...
            gdm = read("etc/gdm3/daemon.conf")
            self.assertNotIn("AutomaticLoginEnable", gdm)
            self.assertNotIn("AutomaticLogin=user", gdm)
    ...
    def test_health_check_rejects_missing_gdm(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-gnome-gdm-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-gnome-gdm-efi-")
        try:
            fake_tree(root)
    ...
    def test_iso_extras_signed_debs_not_live_grub(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-pool-")
        try:
            pool = os.path.join(
            ...
            self.assertEqual(len(find_pool_debs(iso)), 2)
            extras_fn = iso_extras(iso)
            self.assertEqual(extras, extras_fn)
        finally:
            shutil.rmtree(iso, ignore_errors=True)

    def test_bootloader_uses_pool_debs_not_live_grubx64(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-bl-root-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-bl-efi-")
        iso = tempfile.mkdtemp(prefix="fbl-deb13-bl-iso-")
        pkg = tempfile.mkdtemp(prefix="fbl-deb13-bl-pkg-")
        try:
            fake_tree(root)
            for rel in (
                ...
                if os.path.isfile(path):
                    os.unlink(path)
            live_efi = os.path.join(iso, "EFI", "boot")
            os.makedirs(live_efi)
            with open(os.path.join(live_efi, "bootx64.efi"), "wb") as fh:
                fh.write(b"live-shim")
            with open(os.path.join(live_efi, "grubx64.efi"), "wb") as fh:
                fh.write(b"live-gcdx64")

            def make_deb(name: str, files: dict[str, bytes]) -> str:
                tree = os.path.join(pkg, name)
                debian = os.path.join(tree, "DEBIAN")
                os.makedirs(debian)
                with open(os.path.join(debian, "control"), "w", encoding="ascii") as fh:
                    fh.write(
                        f"Package: {name}\nVersion: 1\nArchitecture: amd64\n"
                        "Maintainer: test\nDescription: test\n"
                    )
                for rel, blob in files.items():
                    ...
                    with open(dest, "wb") as fh:
                        fh.write(blob)
                deb = os.path.join(pkg, f"{name}_1_amd64.deb")
                proc = subprocess.run(
                    ["dpkg-deb", "-b", tree, deb],
                    check=False,
                    capture_output=True,
            ...
            grub_deb = make_deb(
                ...
                {
                    "usr/lib/grub/x86_64-efi-signed/grubx64.efi.signed": b"installed-grub"
                },
            )
            shim_deb = make_deb(
                "shim-signed",
                {"usr/lib/shim/shimx64.efi.signed": b"installed-shim"},
            )
            mm_deb = make_deb(
                "shim-helpers-amd64-signed",
                {"usr/lib/shim/mmx64.efi.signed": b"installed-mm"},
            )
            pool_g = os.path.join(iso, "pool", "main", "g", "grub-efi-amd64-signed")
            pool_s = os.path.join(iso, "pool", "main", "s", "shim-signed")
            pool_h = os.path.join(
                iso, "pool", "main", "s", "shim-helpers-amd64-signed"
            )
            os.makedirs(pool_g)
            os.makedirs(pool_s)
            os.makedirs(pool_h)
            shutil.copy2(
                grub_deb, os.path.join(pool_g, "grub-efi-amd64-signed_1_amd64.deb")
            )
            shutil.copy2(
                shim_deb, os.path.join(pool_s, "shim-signed_1_amd64.deb")
            )
            shutil.copy2(
                mm_deb, os.path.join(pool_h, "shim-helpers-amd64-signed_1_amd64.deb")
            )

            extract_signed_efi_debs(iso, root)
            DRIVER.bootloader(root, efi, disk_for(root, efi), iso)

            vendor_grub = os.path.join(efi, "EFI", "debian", "grubx64.efi")
            bootx = os.path.join(efi, "EFI", "BOOT", "BOOTX64.EFI")
            with open(vendor_grub, "rb") as fh:
                self.assertEqual(fh.read(), b"installed-grub")
            with open(bootx, "rb") as fh:
                self.assertEqual(fh.read(), b"installed-shim")
            self.assertFalse(
                os.path.isfile(os.path.join(efi, "EFI", "BOOT", "grubx64.efi"))
            )
            cfg = os.path.join(root, "boot", "grub", "grub.cfg")
            with open(cfg, encoding="utf-8") as fh:
                text = fh.read()
            self.assertIn("root=UUID=ROOT-UUID-2222", text)
            self.assertIn("/boot/vmlinuz-6.12.0-0.deb13", text)
            self.assertNotIn("boot=live", text)
            stub = os.path.join(efi, "EFI", "debian", "grub.cfg")
            with open(stub, encoding="utf-8") as fh:
                stub_text = fh.read()
            self.assertIn("search.fs_uuid ROOT-UUID-2222 root", stub_text)
        finally:
            shutil.rmtree(root, ignore_errors=True)
            shutil.rmtree(efi, ignore_errors=True)
            shutil.rmtree(iso, ignore_errors=True)
