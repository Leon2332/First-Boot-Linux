#!/usr/bin/env python3
"""Debian 13 KDE Plasma native installer."""
...
from firstboot.osinstall.debian_13_plasma import DRIVER, ID  # noqa: E402
...
def fake_tree(root: str) -> None:
    ...
    with open(
        os.path.join(
            root, "etc", "xdg", "autostart", "org.kde.plasma-welcome.desktop"
        ),
        "w",
        encoding="utf-8",
    ) as fh:
...
class Debian13PlasmaTests(unittest.TestCase):
    def test_driver_contract(self) -> None:
        self.assertEqual(ID, "debian-13-plasma")
    ...
    def test_live_squashfs_relpaths(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-plasma-iso-")
        try:
    ...
    def test_configure_writes_customer_not_live_user(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-plasma-root-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-plasma-efi-")
        try:
            fake_tree(root)
            ...
            self.assertNotIn("User=user", sddm)
            self.assertNotIn("Session=plasma", sddm)
            self.assertFalse(
                os.path.isfile(os.path.join(root, "etc", "sddm.conf.d", "live.conf"))
            )
            kde = read("etc/sddm.conf.d/kde_settings.conf")
            self.assertNotIn("User=user", kde)
            self.assertNotIn("Session=plasma", kde)
            self.assertFalse(
                os.path.isfile(
                    os.path.join(root, "etc", "xdg", "autostart", "calamares.desktop")
                )
    ...
    def test_health_check_rejects_missing_sddm(self) -> None:
        root = tempfile.mkdtemp(prefix="fbl-deb13-plasma-sddm-")
        efi = tempfile.mkdtemp(prefix="fbl-deb13-plasma-sddm-efi-")
        try:
            fake_tree(root)
            os.unlink(os.path.join(root, "usr", "lib", "systemd", "system", "sddm.service"))
            ident = OsIdentity("shop-pc", "leon", "Leon", UBUNTU_HASH)
            loc = InstallLocale()
            DRIVER.configure(root, ident, loc, disk_for(root, efi))
            fake_esp(efi)
    ...
    def test_iso_extras_signed_debs_not_live_grub(self) -> None:
        iso = tempfile.mkdtemp(prefix="fbl-deb13-plasma-pool-")
        try:
            pool = os.path.join(
