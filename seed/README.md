...
| labwc + GTK4 chooser (tty1 autologin; GNOME-like panel / QS, not a desktop) + VTE terminal | App Center, ubiquity |
...
The live session is not a desktop. `firstboot-kiosk.service` runs `firstboot-session` as `firstboot` on tty1 (`getty@tty1` is masked). That starts `labwc -S firstboot-chooser`. Ctrl+Alt+F2 is a debug TTY. The chooser paints the mockup chrome (top bar, quick settings, NetworkManager Ethernet/Wi-Fi, power) and reads `/run/payload/catalog.json`, `retailer.conf`, and the two wallpapers. Local vs download is whether the edition file exists under `images/`. USB-boot only: the app grid offers **Install to this device**, which copies ESP + `fbl` + payload onto the internal disk (`firstboot-install-disk`). Local Ubuntu (`ubuntu-autoinstall`) verifies the staged ISO, seeds Subiquity via a one-shot GRUB entry on FBL-SYS (`firstboot-install-os`), and reboots into the Ubuntu installer. Mint is not in this seed yet. The app-grid **Terminal** opens an in-kiosk VTE window (login shell). **System details** opens an in-kiosk window (hardware from sysfs/DMI/PCI, software from os-release). Browser is listed and does not launch.
